<?php
class VipAction extends HomeAction{
    // TAG话题
    ...
    public function vip(){
        ...
        if($_GET['action'] == 'trysee'){
            $this->assign($detail);
            $this->display('Base:vod_player_vip_trysee');
            exit();
        }
        ...
        if($detail['play_status']==500 || $detail['play_status']==501){
            $detail['play_tips'] = $this->fetch('Base:vod_player_vip_login');
        }elseif($detail['play_status']==502){
            $detail['play_tips'] = $this->fetch('Base:vod_player_vip_ispay');
        }elseif($detail['play_status']==503){
            $detail['play_tips'] = str_replace('{vod_price}',$detail['play_price'],$this->fetch('Base:vod_player_vip_price'));
        }else{
        ...
        if($_GET['action'] == "ispay" && $detail['play_status']==503){
            if($user['user_score'] < $detail['play_price']){
                ...
                $detail['play_tips'] = $this->fetch('Base:vod_player_vip_short');
            }else{
