...
<div class="container ff-bg">
  <div class="row ff-row">
    <div class="col-md-8 ff-col ff-slide-pic">
      <?php $item_slide = ff_mysql_vod('field:list_id,list_dir,vod_id,vod_name,vod_ename,vod_jumpurl,vod_pic_slide;cid:'.$list_id.',7;limit:6;pic_slide:true;cache_name:default;cache_time:default;order:vod_stars desc,vod_id;sort:desc'); ?>
...
</dl> <?php echo constant("图片轮播右边类型*");?>
    </div>
  </div>
</div>
<div class="clearfix ff-clearfix"></div>
<?php $_result=explode(',',$list_extend['type']);if(is_array($_result)): $i = 0; $__LIST__ = array_slice($_result,0,8,true);if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$feifeilist): ++$i;$mod = ($i % 2 )?><?php $item_vod=ff_mysql_vod('cid:'.$list_id.';tag_name:'.$feifeilist.';tag_list:vod_type;limit:12;cache_name:default;cache_time:default;order:vod_stars desc,vod_addtime;sort:desc'); ?>
...
<div class="row ff-row">
  <div class="col-md-8 ff-col">
    <div class="page-header">
      ...
      <?php echo ($feifeilist); ?>
      <span class="pull-right"><a href="<?php echo ff_url('vod/type',array('id'=>$list_id,'type'=>urlencode($feifeilist),'area'=>'','year'=>'','star'=>'','state'=>'','order'=>'addtime','p'=>1),true);?>" class="btn btn-success btn-xs">更多</a></span>
      </h2>
    </div>
...
<?php echo ($feifeiarea); ?>
  ...
  <div class="col-md-4 ff-col visible-lg visible-md">
    ...
    <ol class="vod-item-ol">
      <?php $_result=ff_mysql_vod('field:list_id,list_dir,vod_id,vod_cid,vod_name,vod_ename,vod_jumpurl,vod_hits;cid:'.$list_id.';tag_name:'.$feifeilist.';tag_list:vod_type;limit:25;cache_name:default;cache_time:default;order:vod_hits desc');if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$feifei): ++$i;$mod = ($i % 2 )?><li>
