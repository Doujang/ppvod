...
<div class="container ff-bg">
  ...
  <dl class="dl-horizontal">
    ...
    <?php $_result=explode(',',$list_extend['type']);if(is_array($_result)): $i = 0; $__LIST__ = array_slice($_result,0,15,true);if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$feifei): ++$i;$mod = ($i % 2 )?><a href="<?php echo ff_url('vod/type',array('id'=>$type_id,'type'=>urlencode($feifei),'area'=>urlencode($type_area),'year'=>$type_year,'star'=>urlencode($type_star),'state'=>urlencode($type_state),'order'=>$type_order,'p'=>1),true);?>" class="btn btn-sm btn-default gallery-cell" id="type<?php echo md5($feifei);?>"><?php echo ($feifei); ?></a><?php endforeach; endif; else: echo "" ;endif; ?>
    </dd>
    <dt>地区：</dt>
    <dd class="text-nowrap ff-gallery">
    ...
    <?php $_result=explode(',',$list_extend['year']);if(is_array($_result)): $i = 0; $__LIST__ = array_slice($_result,0,7,true);if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$feifei): ++$i;$mod = ($i % 2 )?><a href="<?php echo ff_url('vod/type',array('id'=>$type_id,'type'=>urlencode($type_type),'area'=>urlencode($type_area),'year'=>$feifei,'star'=>urlencode($type_star),'state'=>urlencode($type_state),'order'=>$type_order,'p'=>1),true);?>" class="btn btn-sm btn-default gallery-cell" id="year<?php echo md5($feifei);?>"><?php echo ($feifei); ?></a><?php endforeach; endif; else: echo "" ;endif; ?>
      <a href="<?php echo ff_url('vod/type',array('id'=>$type_id,'type'=>urlencode($type_type),'area'=>urlencode($type_area),'year'=>'20002010','star'=>urlencode($type_star),'state'=>urlencode($type_state),'order'=>$type_order,'p'=>1),true);?>" class="btn btn-sm btn-default gallery-cell" id="year<?php echo md5('20002010');?>">2010-2000</a>
      <a href="<?php echo ff_url('vod/type',array('id'=>$type_id,'type'=>urlencode($type_type),'area'=>urlencode($type_area),'year'=>'19901999','star'=>urlencode($type_star),'state'=>urlencode($type_state),'order'=>$type_order,'p'=>1),true);?>" class="btn btn-sm btn-default gallery-cell" id="year<?php echo md5('19901999');?>">90年代</a>
      <a href="<?php echo ff_url('vod/type',array('id'=>$type_id,'type'=>urlencode($type_type),'area'=>urlencode($type_area),'year'=>'18001989','star'=>urlencode($type_star),'state'=>urlencode($type_state),'order'=>$type_order,'p'=>1),true);?>" class="btn btn-sm btn-default gallery-cell" id="year<?php echo md5('18001989');?>">更早</a>
    </dd>
