...
<div class="modal-dialog">
  <div class="modal-content">
      ...
      <eq name="Think.config.user_pay_name" value="vyipay">
        <form class="form-horizontal user-pay-form" action="{:ff_url('user/pay')}" method="post" role="form" target="_blank" style="padding:20px 0" data-small="{:C('user_pay_small')}">
          <div class="form-group">
            <label for="score_ext" class="col-xs-3 control-label">充值金额</label>
            <div class="col-xs-9">
              <input class="form-control text-center" name="score_ext" id="score_ext" value="{:(C('user_pay_small'))}" type="text" required>
            </div>
          </div>
          <div class="form-group">
            <label for="pay_type" class="col-xs-3 control-label">支付方式</label>
            <div class="col-xs-9 text-left">
              ...
              <label class="radio-inline">
                <input type="radio" name="pay_type" id="pay_type" value="qqpay"> QQ
              </label>
            </div>
          </div>
    ...
    <div class="modal-footer">
      ...
      1元等于<span class="ff-text">{:C("user_pay_scale")}</span>个影币，最低<span class="ff-text">{:C("user_pay_small")}</span>元起充
      </h6>
    </div>
  </div>
</div>
