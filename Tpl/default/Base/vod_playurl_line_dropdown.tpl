...
<div class="tab-content ff-playurl-dropdown">
  ...
  <ul class="list-unstyled row text-center tab-pane ff-playurl ff-playurl-dropdown-{$feifei.player_sid} {$active}" data-active="{$vod_id}-{$play_sid}-{$play_pid}" data-more="{$Think.config.ui_playurl|intval}">
    ...
    <li class="col-md-1 col-xs-4" data-id="{$vod_id}-{$feifei.player_sid}-{$pid}"><a href="{:ff_url_vod_play($list_id,$list_dir,$vod_id,$vod_ename,$feifei['player_sid'],$pid)}" class="btn btn-default btn-block btn-sm ff-text-hidden">{$feifeison.title}</a></li>
    </volist>
  </ul>
  </volist>
</div>
